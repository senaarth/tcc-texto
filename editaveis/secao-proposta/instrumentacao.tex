...
A plataforma fornece bibliotecas que viabilizam o disparo de eventos diretamente no código-fonte do produto, e, além da coleta da métrica em si, também é possível adicionar propriedades específicas ao evento, como informações sobre o usuário, sobre seu comportamento, entre outras. Além disso, o MixPanel fornece uma API de consumo desses dados, permitindo ao desenvolvedor filtrá-los conforme necessário. Esta união de fatores permite que os dados além de computados, carreguem metadados que viabilizem filtragens e manipulações.

No trecho de código abaixo é possível verificar o disparo do evento no lado do cliente, onde o mesmo carrega o nome do evento e os metadados necessários para manipulação e análise do mesmo (linhas 10 a 18).
        ...
        <button
            onClick={() => {
                ...
                Mixpanel.trackWithProperties(
                    ...
                    # como por exemplo qual versao o usuario estava utilizando
                    # ao clicar no botao
                    {
                    'Versao utilizada':
...
# faz a chamada a api do mixpanel e realizada o parser dos dados
response = requests.post(api_url, data=params, auth=HTTPBasicAuth(api_secret, ''))
data = response.text.splitlines()
events = [json.loads(rec) for rec in data]

# deve apresentar uma lista semelhante a seguinte:
# [{'event': 'Botao do experimento clicado', 'properties': {'Versao utilizada': 'Tratamento'}}]
print(events)
\end{lstlisting}
