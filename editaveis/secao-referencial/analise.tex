...
\[
\text{amplitude} = x_{\text{max}} - x_{\text{min}}\text{,}
\]

\[
\textit{s} = \sqrt{s^2}\text{, e}
\]

\[
s^2 = \frac{1}{n} \sum_{i=1}^{n} (x_i - \bar{x})^2\text{.}
\]
...
\subsection{Redução de um Conjunto de Dados}
\label{subsec:reduction}

O processo de redução de dados, como citado anteriormente é crucial para a realização de testes estatísticos e a eficácia destes métodos estatísticos depende da qualidade dos dados de entrada. Se estes não representarem corretamente o que se pretende analisar, as conclusões derivadas dessas análises podem ser incorretas. A eliminação de \textit{outliers}, aumentam a confiabilidade dos resultados e evitam interpretações equivocadas  \cite{wohlin_experimentation_2012}. Para se realizar este processo, existem diferentes métodos estatísticos, abaixo são apresentados dois destes.

\subsubsection{Por Distância Interquartil}

Uma maneira eficaz de identificar pontos de dados extremos é por meio do \textbf{intervalo interquartil}. Utilizando os quartis, é possível detectar valores que se encontram significativamente afastados do restante dos dados. Para realizar a remoção desses pontos, define-se um limite com base no intervalo interquartil, \textit{IQR}, que é calculado subtraindo o primeiro quartil do terceiro, conforme a fórmula abaixo:

\[
IQR = Q_3 - Q_1.
\]

Os limites inferior e superior são então estabelecidos a partir do valor de \textit{IQR}, e quaisquer valores que ultrapassem esses limites são classificados como \textit{outliers}. O cálculo desses limites é dado por:
...
Essa técnica não pressupõe que a distribuição do conjunto seja normal, pois a identificação de \textit{outliers} baseia-se na ordenação dos dados. Por isso, ela apresenta um bom desempenho em conjuntos de dados assimétricos.

\subsubsection{Por Desvio Padrão}

Outra forma de identificar valores extremos em um conjunto é através do método do \textit{Z-Score}, que utiliza \textbf{a média e o desvio padrão da distribuição}. O \textit{Z-Score} mede o quão distante, em termos de desvios padrão, um dado ponto está da média do conjunto. Ele é calculado usando a seguinte fórmula:
...
onde:
\begin{itemize}
    \item \( Z \) é o \textit{Z-Score};
    \item \( x \) é o valor do dado;
    \item \( \mu \) é a média do conjunto de dados;
    \item \( \sigma \) é o desvio padrão da distribuição.
\end{itemize}

Nesse caso, \textit{outliers}, são identificados quando o valor absoluto do \textit{Z-Score} excede um limite pré-determinado. Geralmente, utiliza-se \( |Z| > 3 \) como critério para classificar um valor como \textit{outlier}, pois isso corresponde a pontos que estão a mais de 3 desvios padrão da média.

O método do \textit{Z-Score} é mais adequado para dados que seguem uma distribuição aproximadamente normal, uma vez que a média e o desvio padrão são medidas confiáveis de tendência central e dispersão nesse caso.


\subsection{Normalização de Dados}
\label{subsec:normalization}

A normalização de dados é uma técnica amplamente utilizada em pré-processamento, especialmente em algoritmos de aprendizado de máquina e estatística, para ajustar os valores das variáveis a uma escala específica. Isso é feito para reduzir o impacto de diferentes escalas das variáveis e melhorar a performance de modelos. A escolha do método de normalização depende do objetivo e da distribuição dos dados \cite{Dekking2005}.

\subsubsection{Método Máximo e Mínimo}

O método de normalização \textbf{Máximo e Mínimo} transforma os dados para um intervalo fixo, normalmente entre \( [0, 1] \). A fórmula para a normalização de um dado \( x \) é dada por:
...
\begin{itemize}
    ...
    \item \( x_{\text{min}} \) e \( x_{\text{max}} \) são os valores mínimo e máximo no conjunto de dados.
\end{itemize}

Este método é útil quando é necessário restringir os valores em um intervalo fixo. No entanto, ele é sensível a \textit{outliers}, já que valores extremos podem distorcer a escala dos dados.

\subsubsection{Transformação Box-Cox}

Esta técnica é utilizada para transformar os dados de modo que se aproximem de uma distribuição normal, reduzindo assimetria e estabilizando a variância. A fórmula da transformação é:
...
onde:
\begin{itemize}
    \item \( x \) é o valor original (deve ser estritamente positivo);
    \item \( y \) é o valor transformado;
    \item \( \lambda \) é o parâmetro de transformação, determinado através de diferentes métodos estatísticos para maximizar a normalidade dos dados.
\end{itemize}

A transformação Box-Cox é mais adequada quando os dados apresentam assimetria ou variância não constante. É frequentemente usada em modelos estatísticos, como regressão linear, para atender às suposições de normalidade e homocedasticidade.
...
\[
\bar{x} = \frac{N_1 N_2}{2} \text{, e}
\]
\[
\text{s} = \frac{n_1 n_2 (n_1 + n_2 + 1)}{12} \text{.}
\]

Já o valor \(z\) é calculado como:

\[
z = \frac{u - \bar{x}}{s} \text{.}
\]
