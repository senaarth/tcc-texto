...
Permite uma compreensão aprofundada e contextualizada dos fenômenos de interesse através de investigações dentro de seus contextos reais. Além disso, é flexível, o que possibilita ajustes iterativos em diferentes momentos do processo. Segundo \citeonline{yin_case_study_2009}, os principais tipos de estudo de caso são:

\begin{itemize}
    \item \textbf{Causais ou Explanatórios}: Buscam entender relações de causa e efeito. São utilizados para investigar como e por que certos fenômenos ocorrem, sendo frequentemente empregados em pesquisas que analisam processos complexos e suas interdependências;
    
    \item \textbf{Descritivos}: Têm como objetivo relatar detalhadamente uma situação ou evento específico. Eles fornecem uma visão detalhada do fenômeno estudado sem necessariamente estabelecer relações de causa e efeito. Um estudo descritivo pode, por exemplo, documentar como uma organização implementou uma determinada política ou como uma tecnologia foi adotada em um setor específico; e
    
    \item \textbf{Exploratórios}: Usados principalmente para descobrir novas informações, esses estudos têm caráter preliminar e são úteis quando há pouco conhecimento prévio sobre o tema. Eles podem ajudar a identificar questões relevantes para pesquisas futuras e definir hipóteses para serem testadas posteriormente.
\end{itemize}


Além disso, o autor também define as etapas de realização deste tipo de investigação, são elas: definição, preparação e coleta de dados, análise de dados e relato dos resultados. As mesmas são apresentadas a seguir.

\subsubsection{Definição}

Segundo \citeonline{yin_case_study_2009}, é a etapa inicial, nela são definidas as bases da pesquisa, através do levantamento dos seguintes artefatos:

\begin{itemize}
    \item \textbf{Objetivo:} inicialmente é formulado de forma genérica, com um foco principal que será refinado ao longo do estudo. Define o que o pesquisador pretende alcançar e as perguntas que guiarão a investigação e qual o tipo da mesma;
